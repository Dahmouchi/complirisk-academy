...
<body style="margin: 0; padding: 20px 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; background-color: #f5f7fa;">
    <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.07);">
        ...
        <!-- Header Section -->
        <div style="background: linear-gradient(135deg, #3b82f6 0%, #1d4ed8 100%); padding: 20px 10px; text-align: center; color: #ffffff;">
...
export async function startLiveRoom(liveRoomId: string, teacherId: string) {
  try {
    ...
    // Send email notifications to students in the grade
    if (liveRoom.subject?.gradeId) {
      const students = await prisma.user.findMany({
        where: {
          gradeId: liveRoom.subject.gradeId,
          role: "USER", // Only students
          NOT: { registerCode: null },
        },
        select: {
      ...
      const teacherFullName = `${liveRoom.teacher.prenom || ""} ${
...
<head>
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <title>Live en Direct Maintenant !</title>
</head>
<body style="margin: 0; padding: 20px 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; background-color: #f5f7fa;">
    <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.07);">
        
        <!-- Header Section -->
        <div style="background: linear-gradient(135deg, #10b981 0%, #059669 100%); padding: 20px 10px; text-align: center; color: #ffffff;">
            <div style="margin-bottom: 20px; background-color: #ffffff; padding: 10px; border-radius: 10px;">
                <img src="https://happytrip-test-cinq.puunoo.easypanel.host/logoH.jpg" alt="Logo" style="max-width: 180px; height: auto;">
            </div>
            <h1 style="font-size: 28px; font-weight: 700; margin: 0 0 8px 0; letter-spacing: -0.5px; color: #ffffff;">🔴 Live en Direct !</h1>
            <p style="font-size: 16px; opacity: 0.95; font-weight: 400; margin: 0; color: #ffffff;">Le cours commence maintenant</p>
        </div>
        ...
        <div style="padding: 20px 10px;">
            ...
            <p style="font-size: 16px; color: #4a5568; margin: 0 0 30px 0; line-height: 1.7;">
                Le cours <strong>${liveRoom.name}</strong> vient de commencer ! Ne manquez pas cette opportunité d'apprentissage en direct avec votre enseignant.
            </p>
            
            <!-- Live Session Card -->
            <div style="background: #d1fae5; border: 2px solid #10b981; border-radius: 10px; padding: 25px; margin: 25px 0;">
                <h2 style="font-size: 22px; color: #1a202c; font-weight: 700; margin: 0 0 15px 0;">📚 ${liveRoom.name}</h2>
                
                ${liveRoom.description ? `<p style="font-size: 15px; color: #4a5568; margin: 0 0 20px 0; line-height: 1.6;">${liveRoom.description}</p>` : ""}
                ...
                <table style="width: 100%; border-collapse: collapse;">
                    ...
                    <tr>
                        <td style="padding: 6px 0; font-size: 15px;">
                            <span style="font-size: 18px; margin-right: 12px;">📖</span>
                            <span style="font-weight: 600; color: #2d3748;">Matière :</span>
                            <span style="color: #4a5568;"> ${liveRoom.subject?.name}</span>
                        </td>
                    </tr>
                    <tr>
                        <td style="padding: 6px 0; font-size: 15px;">
                            <span style="font-size: 18px; margin-right: 12px;">🔴</span>
                            <span style="font-weight: 600; color: #2d3748;">Statut :</span>
                            <span style="color: #10b981; font-weight: 600;"> EN DIRECT</span>
                        </td>
                    </tr>
                </table>
            </div>
            ...
            <!-- Call to Action -->
            <div style="text-align: center; margin: 35px 0;">
                <a href="${process.env.NEXTAUTH_URL || "https://cinqcinq.ma"}/dashboard/live" style="display: inline-block; background: linear-gradient(135deg, #10b981 0%, #059669 100%); color: #ffffff; text-decoration: none; padding: 16px 40px; border-radius: 8px; font-size: 16px; font-weight: 600; box-shadow: 0 4px 6px rgba(16, 185, 129, 0.3);">
                    🎥 Rejoindre le Live →
                </a>  
            </div>
            ...
            <div style="background: #fef3c7; border-left: 4px solid #f59e0b; padding: 15px; border-radius: 4px; margin: 20px 0;">
                <p style="margin: 0; font-size: 14px; color: #92400e;">
                    <strong>⚠️ Important :</strong> Le cours a déjà commencé. Rejoignez-le dès maintenant pour ne rien manquer !
                </p>
            </div>
        </div>
